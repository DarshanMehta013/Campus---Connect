...
function filterPublicFeed(dept) {
  ...
  document.querySelectorAll('.feed-filter-btn').forEach(btn => {
    btn.className = 'feed-filter-btn btn-animate px-4 py-2 rounded-xl bg-slate-100 dark:bg-zinc-800 font-semibold';
  });
  if (typeof event !== 'undefined' && event && event.target) {
    event.target.className = 'feed-filter-btn btn-animate px-4 py-2 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-black font-bold';
  }
...
function renderPublicFeed(filter = 'all') {
  ...
  grid.innerHTML = list.map(c => `
    <div class="p-6 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl shadow-sm space-y-4 hover:border-blue-500/40 transition-all card-hover group reveal-on-scroll">
      <div class="flex justify-between items-start">
        <div class="space-y-1">
          ...
          <h4 class="font-display font-bold text-lg text-slate-900 dark:text-white pt-1">${c.title}</h4>
        </div>
      ...
      <p class="text-sm text-slate-600 dark:text-zinc-300 leading-relaxed font-normal">${c.description || c.desc || ''}</p>

      <div class="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-slate-100 dark:border-zinc-800 text-slate-500 dark:text-zinc-400">
      ...
      ${c.image ? `
        <div class="pt-2">
          ...
          <img src="${c.image}" onclick="openLightbox('${c.image}', '${c.title}')" class="h-28 w-full object-cover rounded-2xl border cursor-pointer hover:opacity-90 transition">
        </div>
      ` : ''}

      ${c.proofImg ? `
        <div class="p-3.5 rounded-2xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-500/20 flex items-center justify-between">
